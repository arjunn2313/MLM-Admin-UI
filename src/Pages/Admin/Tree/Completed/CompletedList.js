  ...
  const fetchSectionData = async () => {
    ...
    try {
      const response = await axios.get(
        `${BaseUrl}/api/admin/section/completed-members/${treeName}`,
        {
          params: {
            search: searchQuery,
            page: currentPage,
              ...
              <tbody>
                {members?.length > 0 ? (
                  members.map((member, index) => (
                    ...
                    >
                      <td className="p-2 py-4 text-left"> {(currentPage - 1) * 9 + index + 1}</td>
                      ...
                      <td
                        ...
                        onClick={() =>
                          navigate(
                            `?tab=CompletedList&tree=completedtree&memberId=${member?.memberId}&memberName=${member?.name}&sponsorId=${member?.sponsorId}&headName=${member?.sponsorName}`
                          )
                        }
                      >
                        <img
                          ...
                          alt="tree-icon"
                          className="bg-orange-50 p-1 rounded-lg cursor-pointer"
                        />
                      </td>
                    </tr>
                  ))
